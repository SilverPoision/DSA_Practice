#include <iostream>
#define MAX_CHAR 256
using namespace std;

struct node
{
  char a;
  struct node *next, *prev;
};

// Function to append a character x at the end
// of DLL.
// Note that the function may change head and tail
// pointers, that is why pointers to these pointers are passed.
void appendNode(struct node **head_ref, struct node **tail_ref, char x)
{
  node *temp = new node;
  temp->a = x;
  temp->next = temp->prev = NULL;

  if (*head_ref == NULL)
  {
    *head_ref = *tail_ref = temp;
    return;
  }

  (*tail_ref)->next = temp;
  temp->prev = (*tail_ref);
  *tail_ref = temp;
}

// function to remove a node 'temp' fromt DLL.
//  Note that the function may change head and tail pointers,
//  that is why pointers to these pointers are passed.
void removeNode(struct node **head_ref, struct node **tail_ref, struct node *temp)
{

  if (*head_ref == NULL)
  {
    return;
  }

  if (*head_ref == temp)
  {
    *head_ref = (*head_ref)->next;
  }

  if (*tail_ref == temp)
  {
    (*tail_ref)->prev = *tail_ref;
  }

  if (temp->next && temp->prev)
  {
    temp->prev->next = temp->next;
    temp->next->prev = temp->prev;
  }

  delete (temp);
}

void findFirstNonRepeating()
{
  // arr[x] contains pointer to
  // a DLL node if x is present
  // in DLL. If x is not present, then arr[x] is NULL
  struct node *arr[MAX_CHAR];

  // repeated[x] is true if x is repeated two or more times.
  // If x is not seen so far or x is seen only once. then
  // repeated[x] is false
  bool repeated[MAX_CHAR];

  // Initialize the above two arrays
  node *head = NULL, *tail = NULL;
  for (int i = 0; i < MAX_CHAR; i++)
  {
    arr[i] = NULL;
    repeated[i] = false;
  }

  // Let us consider following stream and see the process
  char stream[] = "ABCDBAGHC";
  for (int i = 0; stream[i]; i++)
  {
    char x = stream[i];
    cout << "Reading " << x << " from stream n";

    // We process this character only if it has not occurred
    // or occurred only once. repeated[x] is true if x is
    // repeated twice or more.s
    if (!repeated[x])
    {
      // If the character is not in DLL, then add this at
      // the end of DLL.
      if (arr[x] == NULL)
      {
        appendNode(&head, &tail, stream[i]);
        arr[x] = tail;
      }
      else // Otherwise remove this character from DLL
      {
        removeNode(&head, &tail, arr[x]);
        arr[x] = NULL;
        repeated[x] = true; // Also mark it as repeated
      }
    }

    // Print the current first non-repeating character from
    // stream
    if (head != NULL)
      cout << "First non-repeating character so far is "
           << head->a << endl;
  }
}

int main()
{
  findFirstNonRepeating();
  return 0;
}